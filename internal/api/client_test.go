...
// Existe um cliente único e ele tem deadline. Sem isto, o poll síncrono do agente pendura
// para sempre numa conexão morta.
...
// URL inválida vira erro tratado. O código anterior fazia `req, _ := http.NewRequest(...)` e
...
// Método inválido também sai como erro, não pânico.
...
// Context nulo é recusado explicitamente — um caminho sem context é um caminho sem
...
// Servidor que nunca responde não congela o agente — o deadline do context corta.
...
// O token vira `Authorization: Bearer` uma vez só, no cliente — nenhuma tela repete a
// montagem do header.
...
// Sem token, nenhum header de autorização vazio é enviado.
...
// O 429 real de `GET /pending-jobs` traz Retry-After **e** pollIntervalMs; o header vence.
...
// A forma data-absoluta do Retry-After também é aceita.
...
// Unauthorized só é verdadeiro em 401 — é o sinal que separa "insistir com backoff" (rede
...
// O corpo grande (PDF de job) sai por Open, sem passar pela memória.
